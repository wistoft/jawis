...
import { getPromise, pres, unknownToErrorData } from "^jab";
import {
...
import { clearModuleCache } from "^jab-node";
 ...
 */
export class MochaAdapter implements TestFrameworkProv<MinimalTestId> {
  constructor(private deps: TestFrameworkDeps) {}

  /**
   * Tests are load by doing a dry run.
   *
   * todo: cache discovered tests, and watch files for change to avoid a lot of work.
   *
   */
   ...
   *
   */
  public runTest = async (globalId: string, testId: MinimalTestId) => {
    const deps: ReporterDeps = {} as any;

    await this.runMocha({
      reporter: makeMyReporter(deps),
      grep: new RegExp("^" + escapeStringRegexp(testId.name) + "$"),
    });
