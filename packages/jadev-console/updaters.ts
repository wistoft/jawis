...
import { parseBacktrace, parseBacktraceAndSourceMap } from "^jawis-util/web";
import { ConsoleEntry, UiEntry, UiErrorEntry } from "./types";
import { State } from "./useJadevState";
...
export const getErrorLogUpdateHelper = (
  ...
  if (doSourceMap) {
    ...
    return {
      sync: syncEntry,
      async: parseBacktraceAndSourceMap(entry.data.stack).then(
    ...
    return {
      sync: {
        ...
        data: {
          msg: entry.data.msg,
          info: entry.data.info,
          parsedStack: parseBacktrace(entry.data.stack),
        },
      },
    };
  }
