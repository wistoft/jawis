...
export const makeRogueUpdater = (
  ...
  if (!oldTest.testLogs) {
    console.log("not impl: rogue data received before test report", rogueData);
    return {};
  }
...
export const makeTestCaseUpdater = (
  ...
  const update2 = getShowTestOnTestChangeUpdate(testCase, old, getRandomToken);
 ...
 */
export const onPrevUpdater = (getRandomToken: () => number) => (
  old: State
): Partial<State> => {
 ...
 * Show previous test case
 */
export const onNextUpdater = (getRandomToken: () => number) => (
  old: State
): Partial<State> => {
  if (old.tests === undefined || old.tests.flatIds.length === 0) {
