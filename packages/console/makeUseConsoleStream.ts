...
export const defaultWindowPropertyName = "__JawisConsoleCapture";
...
export const makeUseConsoleStream = (
  windowProperty = defaultWindowPropertyName
) => {
  ...
  const fireEventBatch = () => {
    ...
    if (dataSeen >= maxEntries) {
      ...
      captureCache.data.push({
        ...
        logName: "control",
        context: "browser",
        data: ["Console reached max entries (" + maxEntries + "): ", dataSeen],
      });
    }
