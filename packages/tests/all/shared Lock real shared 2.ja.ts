import { assert, makeSend } from "^jab";

import { TestProvision } from "^jarun";
import { getLock, makeLockTestMain, startOtherLock } from "^tests/_fixture";

//get shared when other thread initially holds exclusive

export default async (prov: TestProvision) => {
  const lock = getLock(prov);

  const key = lock.getExclusive();

  assert(lock.isExclusiveLocked(), "Should be locked");

  const onMessage = () => {
    console.log("releasing");
    lock.releaseExclusive(key);
  };

  await startOtherLock(prov, __filename, lock, onMessage);

  assert(lock.isShareLocked(), "Should be locked"); //the other thread didn't release
};

/**
 *
 */
export const main = makeLockTestMain((lock) => {
  assert(lock.isExclusiveLocked(), "Should be locked");

  makeSend()({}); //make the other release.

  lock.getShared();

  assert(lock.isShareLocked(), "Should be locked");

  console.log("got the lock");
});
